...
segment_sacrum_nnUNet(){
  ...
  "${PATH_NNUNET_ENV}"/bin/python3 "${PATH_REPO}"/src/totalsegmri/utils/run_nnunet_inference_single_subject.py -i "${file_in}" -o "${file_out}" -path-model "${PATH_NNUNET_MODEL}" -fold "${FOLD}" -use-gpu
}
...
generate_json(){
  ...
  # Call python function
  # TODO: the hard-coded path to the conda environment is not ideal.
  "${PATH_NNUNET_ENV}"/bin/python3 "${PATH_REPO}"/src/totalsegmri/utils/create_jsonsidecar.py -path-json "${path_json}" -process "${process}" -author "${author}"
}
