...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=textwrap.dedent(f'''
            ...
            2. Leave 25% of discs as is.
            3. 50% of discs with single voxel.
            4. Randomally choose disc voxels.
            Output 3 channels (1,2,3) for label 10 to 22 with step of 2.
        '''),
...
def generate_random_disc_labels(seg_path, add_to_discs_labels, output_path, seg_suffix, output_seg_suffix):
    ...
    seg_data = seg_data + add_to_discs_labels
    
    # Randomly decide whether to limit the label to a single voxel for 50% of files
    single_voxel = np.random.choice(2)

    # Filter out labels that are not in the range of 3 - add_to_discs_labels to 25 - add_to_discs_labels
    seg_data_out = np.zeros_like(seg_data)

    for d in range(10, 22 + 1, 2):
        ...
        if not single_voxel and np.random.choice(2, p=(0.75, 0.25)):
    ...
    l = list(range(10, 22 + 1, 2))
    for i in range(3):
        seg_data_out_curr = np.zeros_like(seg_data)
        seg_data_out_curr[np.isin(seg_data_out, [x for x in l if f"{int((x-8)/2):03b}"[2 - i]=="1"])] = 1
        # Create result segmentation
        seg = nib.Nifti1Image(seg_data_out_curr, seg.affine, seg.header)
        seg.set_data_dtype(np.uint8)
        # Make sure output directory exists
        output_seg_path.parent.mkdir(parents=True, exist_ok=True)
        # Save mapped segmentation
        nib.save(seg, str(output_seg_path).replace('.nii.gz', f'_000{i + 1}.nii.gz'))
